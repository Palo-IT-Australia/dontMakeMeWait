    ...
    that.socket.addEventListener('message', function (event) {
      ...
      renderMessage: function(message) {
        if (!that.next_line) {
          ...
          that.next_line.className = "message_template" + (message.type? " " + message.type : '');
          that.message_list.appendChild(that.next_line);
        }
        if (message.text) {
          message.html = converter.makeHtml(message.text);
        }
        message.displayAvatar = message.type !== 'outgoing';
...
(function () {
  navigator.geolocation.getCurrentPosition(function (position) {
    Botkit.boot({}, { lat: position.coords.latitude, lon: position.coords.longitude });
  }, function (err) {
    Botkit.boot({}, null);
  });
})();
